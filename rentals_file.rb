class RentalsFile
  ...
  def read
    ...
    deserialized_rentals.each do |rental|
      ...
      when 'Student'
        person = Student.new(rental['person']['age'], rental['person']['classroom'], rental['person']['name'],
                             parent_permission: rental['person']['parent_permission']).modify_id(rental['person']['id'])
      when 'Teacher'
        person = Teacher.new(rental['person']['age'], rental['person']['specialization'],
                             name: rental['person']['name']).modify_id(rental['person']['id'])
      end
